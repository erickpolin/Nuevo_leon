...
ConcNuevo_leon$Nhog <- 1

#DECILES 

#Attaching the data frame
attach(ConcNuevo_leon) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcNuevo_leon<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcNuevo_leon) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcNuevo_leon$factor,to.data.frame=TRUE)
...
ConcNuevo_leon$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcNuevo_leon$MAXT<-ConcNuevo_leon$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcNuevo_leon<-ConcNuevo_leon[with(ConcNuevo_leon, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcNuevo_leon$ACUMULA<-cumsum(ConcNuevo_leon$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcNuevo_leon[dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcNuevo_leon<-rbind(ConcNuevo_leon[1:(dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1]+1),],
                     ConcNuevo_leon[(dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1]+1):dim(ConcNuevo_leon[1])[1],])
  
  b1<-tam_dec*i-ConcNuevo_leon[dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcNuevo_leon[(dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcNuevo_leon[(dim(ConcNuevo_leon[ConcNuevo_leon$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcNuevo_leon$ACUMULA2<-cumsum(ConcNuevo_leon$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcNuevo_leon$DECIL<-0
...
ConcNuevo_leon[(ConcNuevo_leon$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcNuevo_leon[((ConcNuevo_leon$ACUMULA2>tam_dec*i)&(ConcNuevo_leon$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcNuevo_leon[ConcNuevo_leon$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Nuevo_leon")
write.dbf(ConcNuevo_leon,file="ConcNuevo_leon2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Nuevo_leon")
write.dbf(ConcNuevo_leon,file="ConcNuevo_leon2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Nuevo_leon")
Conc2010<-read.dbf("ConcNuevo_leon2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Nuevo_leon")
Conc2018<-read.dbf("ConcNuevo_leon2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Nuevo_leon NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Nuevo_leon NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Nuevo_leon NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Nuevo_leon NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Nuevo_leon NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Nuevo_leon por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Nuevo_leon Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Nuevo_leon NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Nuevo_leon NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Nuevo_leon INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Nuevo_leon INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
